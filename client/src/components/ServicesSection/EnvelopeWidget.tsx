...
import { useTransform, useMotionValueEvent, motionValue, motion } from "motion/react"
import { useStickyScrollProgress } from "@/components/ui/sticky-scroll-reveal"
import { useState } from "react"

// Fully open well before the card switches (~0.167)
...
export function EnvelopeWidget({ keywords }: { keywords: string[] }) {
    ...
    const letterY   = useTransform(scrollProgress, [OPEN_START, OPEN_END], ["0px", "-58px"])
    const sealOp    = useTransform(scrollProgress, [OPEN_START, OPEN_END * 0.6], [1, 0])
    const sealScale = useTransform(scrollProgress, [OPEN_START, OPEN_END * 0.6], [1, 0])
    const sealRot   = useTransform(scrollProgress, [OPEN_START, OPEN_END * 0.6], [0, 180])

    const [flapOpen, setFlapOpen] = useState(0)
    useMotionValueEvent(scrollProgress, "change", (v) => {
        const t = Math.min(Math.max((v - OPEN_START) / (OPEN_END - OPEN_START), 0), 1)
        setFlapOpen(t)
    })
    ...
    return (
        ...
        >
            {/* Card: wider + explicit height for readable content */}
            <div className="relative flex items-center justify-center"
                ...
                {/* Wax seal — shows logo when closed */}
                <motion.div
