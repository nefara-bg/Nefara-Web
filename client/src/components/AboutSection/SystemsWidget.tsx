...
export default function SystemsWidget() {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            const TOTAL = 2.4
            const tf = (frac: number) => frac * TOTAL
            ...
            tl.to([borderTopRef.current, borderBotRef.current],
                { scaleX: 1, duration: tf(LINE_END - LINE_START), ease: "power2.out" },
                tf(LINE_START))

            // Triangle rotation + label counter-rotation
            tl.to(groupRef.current,
                {
                    rotation: ROTATE_DEG,
                    svgOrigin: `${CX} ${CY}`,
                    duration: tf(ROTATE_END - ROTATE_START),
                    ease: "power1.inOut",
                    onUpdate() {
            ...
            nodeRefs.current.forEach((el, i) => {
                if (!el) return
                tl.to(el,
                    { opacity: 1, scale: 1, transformOrigin: "center center", duration: tf(NODE_ENDS[i] - NODE_STARTS[i]), ease: "back.out(1.4)" },
                    tf(NODE_STARTS[i]))
            })
            ...
            edgeRefs.current.forEach((el, i) => {
                if (!el) return
                tl.to(el,
                    { strokeDashoffset: 0, duration: tf(EDGE_ENDS[i] - EDGE_STARTS[i]), ease: "power2.out" },
                    tf(EDGE_STARTS[i]))
            })

            // Center label
            tl.to(centerRef.current,
                { opacity: 1, scale: 1, transformOrigin: "center center", duration: tf(CENTER_IN[1] - CENTER_IN[0]), ease: "power2.out" },
                tf(CENTER_IN[0]))
        }, root)
            ...
            <div className="flex items-center justify-center px-4 py-6">
                ...
                >
                    ...
                    <g ref={groupRef}>
                        ...
                        {/* Product nodes */}
                        {labels.map((label, i) => (
                            <g key={label} ref={el => { nodeRefs.current[i] = el }}>
