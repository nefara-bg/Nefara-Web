...
describe('contact.ts (Actions Layer) - contactAction', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });

    // Helper function to create mock FormData
    const createMockFormData = (fields: Record<string, string | null | undefined>) => {
    ...
    describe('Happy Path Tests', () => {
        it('TC1: Should successfully handle form submission with valid data', async () => {
            ...
            (contactService.sendEmail as any).mockResolvedValue({ success: true });
    ...
    describe('Missing Field Tests', () => {
        it('TC7: Should handle missing email field', async () => {
            ...
            (contactService.sendEmail as any).mockResolvedValue({
                success: false,
    ...
    describe('Parameter Passing Tests', () => {
        it('TC14: Should pass email parameter correctly', async () => {
            ...
            // Assert
            const callArgs = (contactService.sendEmail as any).mock.calls[0];
        ...
        it('TC17: Should pass null values when fields are missing', async () => {
            // Arrange
            const formData = new FormData();
            (contactService.sendEmail as any).mockResolvedValue({ success: false });
    ...
    describe('Return Value Tests', () => {
        it('TC18: Should return success result from service layer', async () => {
            ...
            (contactService.sendEmail as any).mockResolvedValue(serviceResponse);
        ...
        it('TC19: Should return error result from service layer', async () => {
            ...
            (contactService.sendEmail as any).mockResolvedValue(errorResponse);
    ...
    describe('Error Propagation Tests', () => {
        it('TC25: Should propagate service layer exceptions', async () => {
            ...
            (contactService.sendEmail as any).mockRejectedValue(serviceError);
